...
import { StorageError, type StorageErrorType } from '@dicorre/shared/types/effects'
  ...
  Effect.gen(function* () {
          ...
          for (const series of study.series) {
            ...
            for (const file of series.files) {
              ...
              if (availableFileIds.includes(fileId)) {
                const fileData = yield* fileStorage.loadFile(fileId)
                ...
                yield* Effect.logWarning(`File not found in storage: ${fileId}`)
              }
            }
          }
        }
    ...
    const packageStudiesForDownload = (studies: DicomStudy[], studyIds: string[]) =>
      Effect.gen(function* () {
        ...
        const availableFileIds = yield* fileStorage.listFiles
