...
import type { DicomServerConfig } from '@dicorre/shared/services/config/schema'
import { CancelledError, NetworkError, ValidationError, type DicomSenderError, type StorageErrorType } from '@dicorre/shared/types/effects'
import { FileStorage } from '@dicorre/shared/services/fileStorage'
import { DicomSplitError, inspectMultiframeSplit, splitMultiframeDicom, type SplitRefusalReason } from '@dicorre/shared/services/dicomSplitter'
...
export class DicomSender extends Context.Tag("DicomSender")<
  DicomSender,
  {
    ...
    ) => Effect.Effect<SendStudyResult, DicomSenderError | StorageErrorType, FileStorage | DicomSender>
  }
>() { }
        ...
        const sendEffects = filesToSend.map((file) =>
          Effect.gen(function* () {
            ...
            while (attempts <= MAX_RETRIES) {
              ...
              const loaded = yield* fileStorage.loadFile(file.id).pipe(
